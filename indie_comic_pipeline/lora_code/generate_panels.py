"""
SDXL + LORA EMOTION-AWARE PANEL GENERATOR - T4 OPTIMIZED
...
print("=" * 70)
print("SDXL + LORA EMOTION-AWARE PANEL GENERATOR - T4 OPTIMIZED")
print("Drawing the story with efficient memory management")
print("=" * 70)
...
def get_pipeline(settings, device, force_reload=False):
    ...
    print("\n  Loading SDXL pipeline (first time, caching for future pages)...")
    ...
    try:
        ...
        # Set LoRA adapter scale (lower = faster, slightly less style)
    ...
    if device == "cuda":
        ...
        if hasattr(pipe, "disable_sequential_cpu_offload"):
            # We already have CPU offload, don't need this
            pass
...
for i, panel in enumerate(panels):
    ...
    print(f"  Prompt: {augmented[:100]}...")

    # In enriched mode, trigger words are already in the augmented_prompt
    if USING_ENRICHED_MODE:
        prompt_str = augmented
    else:
        lora_settings = settings.get("models", {}).get("lora", {})
    ...
    try:
        ...
        try:
            ...
            panel_path = get_output_path(comics_dir, f"page_{args.page}_panel_sdxl_lora_{p_num}_fallback.png")
            image.save(panel_path)
            generated_paths.append(panel_path)
