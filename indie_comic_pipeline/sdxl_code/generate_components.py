"""
SDXL BASE COMIC COMPONENT GENERATOR - T4 OPTIMIZED
Generates individual visual assets/components (main character pose, extra person, environment background, props)
Optimized for T4 GPU with memory management and model caching (Base SDXL - No LoRA)
"""
...
print("=" * 70)
print("SDXL BASE COMIC COMPONENT GENERATOR - T4 OPTIMIZED")
print("Generating assets with efficient memory management (Base SDXL)")
print("=" * 70)
...
def get_pipeline(settings, device, force_reload=False):
    ...
    print("\n  Loading SDXL Base pipeline (first time, caching for future use)...")
...
if USING_ENRICHED_MODE:
    ...
    if panels:
        # Component 1: Main character pose (from first panel)
        ...
        # Component 2: Side character (if available)
        ...
        if side_chars:
            ...
            # Fallback side character
            components.append({
                "name": "Mysterious Figure",
                "type": "secondary_character",
                "description": "An enigmatic silhouette watching from the shadows",
                "sdxl_prompt": "indie comic style illustration, clean minimalist line art, flat color palette, mysterious figure, dark silhouette"
            })
        
        # Component 3: Environment/Scenery
        components.append({
        ...
        # Component 4: Recurring motif as prop
...
# IP-Adapter for character consistency (optional, disabled by default on T4)
...
for i, component in enumerate(components):
    ...
    try:
        ...
        component_path = get_output_path(comics_dir, f"component_sdxl_base_{i+1}.png")
        ...
        if (i + 1) % clear_interval == 0:
            print(f"  🧹 Clearing GPU cache (every {clear_interval} components)...")
            clear_gpu_memory()
        ...
        try:
            ...
            component_path = get_output_path(comics_dir, f"component_sdxl_base_{i+1}_fallback.png")
            image.save(component_path)
            component_paths.append(component_path)
            print(f"  ✓ Saved fallback component to: {component_path}")
...
if component_paths:
    ...
    strip_path = get_output_path(comics_dir, "component_sheet_sdxl_base_horizontal.png")
    ...
    if num_assets <= 3:
        ...
        import math
        cols = math.ceil(num_assets / 2.0)
        grid_size = (2, cols)
    
    grid_path = get_output_path(comics_dir, "component_sheet_sdxl_base_grid.png")
    create_comic_grid(component_paths, grid_path, grid_size=grid_size, cell_size=(width, height))
    print(f"  ✓ Grid layout ({grid_size[0]}x{grid_size[1]}): {grid_path}")
else:
    print("Error: No components were successfully generated.")
    sys.exit(1)

# Run character consistency check for main component (if reference exists)
if not USING_ENRICHED_MODE and os.path.exists(ref_path) and component_paths:
    print("\n🔍 Running character consistency check...")
    try:
        ...
        print("-" * 50)
        print(f"Main Character Consistency ({basename}):")
...
print("   - component_sdxl_base_1.png through component_sdxl_base_N.png")
print("   - component_sheet_sdxl_base_horizontal.png")
print("   - component_sheet_sdxl_base_grid.png")
print("=" * 70)